...
./scripts/quality/verify.sh          # сборка, тесты, контракты, линтеры
...
Набор гейтов `verify.sh` лежит в `.quality/quality.config.json` и правится там, не в
скрипте и не в workflow. `apps/landing` в проверки не входит — его держит
...
| Перегенерация кода из контрактов | `scripts/quality/openapi-generate.sh` (C#), `scripts/quality/frontend-openapi-generate.sh` (TS) |
| Публичные HTTP-пути и их покрытие | `backend/Bugget.IntegrationTests/Contract/PublicContractInventory.cs` |
| Как фронт ходит в API | `scripts/quality/frontend-api-inventory.mjs` |
...
(ADR-0016). `backend-coverage` держит покрытие не ниже снимка `.quality/backend-coverage.json`.
