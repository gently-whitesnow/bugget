...
Что за продукт, что внутри, как поднять и настроить — [README.md](README.md).

Продуктом пользуются у заказчика, и у API ровно один клиент — фронтенд этого же
монорепозитория. Отсюда: **публичный HTTP-контракт ломать нельзя.** Если ломаем — фронт
и бекенд правятся одним PR, решение фиксируется ADR.
...
Тело каждого коммита — четыре блока: почему код появился, решение, отброшено,
ограничения. Заготовка — [`.gitmessage`](.gitmessage), полностью —
