...
**Публичный HTTP-контракт зафиксирован снимками.** Запрос, статус, media type и форма
тела каждого пути, который зовёт фронт или nginx, лежат текстом в
...
**На провод не уходит то, чего не описывает контракт.** Инвариант держит гейт
`backend-contract-snapshots` (`scripts/quality/contract-snapshots.py`): он сверяет
снимки — то, что приложение действительно отдаёт — со схемами ответов в
`specs/contracts/**/openapi.yaml` и `shared.yaml`. Поле снимка, которого нет в схеме;
`required`-поле схемы, которого нет в снимке; разошедшийся тип (включая `null` у
не-`nullable`); статус или media type, не объявленный в контракте, — красный гейт.
Операция находится по строке `request:` снимка: это метод и шаблон маршрута из таблицы
маршрутов живого хоста, а не догадка по имени файла. Чинится это правкой контракта
(если поле должно уходить) или кода (если не должно) — не подгонкой снимка. Пути, у
которых схемы ответа нет по существу (handshake SignalR), перечислены поимённо в
`EXCEPTIONS` скрипта; список только сокращается — описанный в контракте путь обязан из
него исчезнуть.
