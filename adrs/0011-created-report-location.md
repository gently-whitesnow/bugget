...
Workspace и team — обязательный контекст работы приложения. Их identity поступает через
заголовки, имена которых задают соответственно
`ExternalSettings:Authentication:OrganizationIdHeaderName` (organization отображается во
внешний workspace) и `ExternalSettings:Authentication:TeamIdHeaderName`. Пустое имя любого
из этих заголовков считается ошибкой конфигурации и останавливает приложение на старте.
...
- Contract-гейт проверяет, что nginx принимает и срезает тот же внешний workspace/team-
  префикс, который backend возвращает в `Location`.
