...
/// <summary>Источник RSA-пар для подписи JWT: читает файл, а если его нет — генерирует пару и сохраняет по тому же пути.</summary>
/// <remarks>
/// Ключи — секрет, в репозитории не лежат. Путь (<c>KeyStoreOptions.PemFilePath</c>) должен указывать на постоянный том:
/// новый ключ инвалидирует все выданные токены, то есть разлогинивает все сессии.
/// </remarks>
