    ...
    IOptions<ReportAliasOptions> aliasOptions)
{
    ...
    [Description(
        ...
        "токена, при test вернёт репорт тому, кто вёл его до правок (или автору " +
        "репорта). Отвечает обновлённым репортом: статус, ответственный, " +
        "момент изменения.")]
    public async Task<string> PatchReportAsync(
        ...
        [Description(
            ...
            "test (запушил правки, репорт уходит на проверку).")] string status)
    {
