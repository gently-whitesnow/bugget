    ...
    IOptions<ReportAliasOptions> aliasOptions)
{
    ...
    /// <summary>
    /// Upload attachment к Bug'у из internal-контекста (beta-bot) — без alias-резолва:
    /// caller уже прошёл ACL на уровне сети и знает `reportId`+`bugId`. TECHSPEC §4.3.2.
    /// Переиспользует общий `SaveAsync` pipeline (валидация, лимиты, storage, events).
    /// `aliasId` для SignalR-группы резолвится через серверный `ReportAliasMode`,
    /// чтобы push дошёл до клиентов в Default/Guid/Team-режимах.
    /// </summary>
    public Task<MonadeStruct<AttachmentDbModel>> SaveBugAttachmentInternalAsync(
        ...
        CancellationToken ct)
    {
        var syntheticUser = new UserIdentity(new System.Security.Claims.ClaimsPrincipal())
        {
            Id = creatorUserId,
        ...
        Task<Error?> validateLimit() => limitsService.ValidateBugAttachmentLimitAsync(reportId, bugId, attachType);

        var aliasId = ReportIdResolveHelper.ToAliasId(reportId, publicId, teamReportId, aliasOptions.Value);

        return SaveAsync(
            user: syntheticUser,
            reportIdContext: new ReportIdContext(reportId, aliasId, creatorTeamId),
            entityId: bugId,
            validateLimit: validateLimit,
            fileStream: fileStream,
            attachType: attachType,
    ...
    /// <summary>
    /// Upload attachment к Comment'у из internal-контекста (beta-bot, диалог тестера).
    /// Caller (бот) знает только `commentId` — workspace/report резолвятся через
    /// `CommentsDbClient.GetCommentLocatorAsync`. `aliasId` для SignalR-группы — через
    /// серверный `ReportAliasMode`. Симметрично `SaveCommentAttachmentAsync` (web-flow).
    /// </summary>
    public Task<MonadeStruct<AttachmentDbModel>> SaveCommentAttachmentInternalAsync(
        string creatorUserId,
        int reportId,
        Guid publicId,
        int? teamReportId,
        ...
        CancellationToken ct)
    {
        ...
        Task<Error?> validateLimit() => limitsService.ValidateCommentAttachmentLimitAsync(creatorUserId, reportId, bugId, commentId);
    ...
    /// <summary>
    /// Upload attachment к BugStep'у из internal-контекста (beta-bot) — без alias-резолва:
    /// caller уже прошёл ACL и знает `reportId`+`bugId`+`stepId`. Симметрично
    /// `SaveBugAttachmentInternalAsync`. См. TECHSPEC §4.3.2 (stepId target).
    /// </summary>
    public Task<MonadeStruct<AttachmentDbModel>> SaveBugStepAttachmentInternalAsync(
        string creatorUserId,
        int reportId,
        Guid publicId,
        int? teamReportId,
        ...
        CancellationToken ct)
    {
        ...
        Task<Error?> validateLimit() => limitsService.ValidateBugStepAttachmentLimitAsync(reportId, bugId, stepId);
