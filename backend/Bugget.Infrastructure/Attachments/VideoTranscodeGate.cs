...
/// <summary>
/// Единственный на процесс потолок одновременных видеозадач. Слот берётся до любой тяжёлой подготовки
/// (временная копия, запуск ffmpeg), иначе ожидающие задачи размножают копии и процессы,
/// и потолок памяти становится фиктивным (MAIN-188).
/// </summary>
public sealed class VideoTranscodeGate
{
    ...
    /// <summary>Лизинг обязан быть освобождён: на <see cref="Lease.Dispose"/> пишется длительность и исход.</summary>
