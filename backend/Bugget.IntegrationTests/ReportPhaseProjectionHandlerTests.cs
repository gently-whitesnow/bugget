...
/// <summary>
/// События вставляются в <c>domain_events</c> через UoW (тот же путь, что и прод-эмиссия), затем гоняем
/// <see cref="DomainEventsPoller.TickAsync"/> и читаем <c>report_phase_intervals</c>: контракт handler'а через реальный poller.
/// </summary>
...
public sealed class ReportPhaseProjectionHandlerTests : IClassFixture<AppWithPostgresFixture>
{
    ...
    public async Task Idempotent_Reprocessing()
    {
        ...
        // Откатываем cursor на тот же tail и гоняем повторно: UNIQUE(source_event_id) на INSERT и idempotent UPDATE
        // не должны породить новых строк.
    ...
    // Репорт создаётся через DA, чтобы FK report_phase_intervals.report_id был валиден; baseTime — нулевой момент для occurred_at.
    ...
    // Cursor консьюмера ставится на текущий MAX(id), чтобы не подхватывать хвост прошлых тестов shared-collection;
    // возвращает tail — тест идемпотентности сбрасывает cursor сюда.
...
RETURNING id;";
    ...
    // Spy: фиксирует, какие EventType прошли через handler (poller диспатчит по EventType).
