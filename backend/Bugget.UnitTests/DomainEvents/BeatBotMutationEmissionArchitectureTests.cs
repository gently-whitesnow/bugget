...
public class BetaBotMutationEmissionArchitectureTests
{
    ...
    [Fact(DisplayName = "Bugget.Application/** не импортирует Npgsql: DA-specifics не должны течь в BO-слой")]
    ...
    private static string LocateBoSourceRoot()
    {
        ...
        Assert.NotNull(dir);
        var bo = Path.Combine(dir!.FullName, "Bugget.Application");
        Assert.True(Directory.Exists(bo), $"Bugget.Application source root not found at {bo}");
        return bo;
    }
}
