...
/// <summary>
/// Enum'ы reports уходят на провод строкой из контракта. Генераторный <c>JsonStringEnumConverter&lt;T&gt;</c> пишет имя
/// CLR-члена (<c>tg_beta_tester</c> → <c>Tg_beta_tester</c>), и совпадение со строкой контракта — случайность.
/// Опции берутся из настоящей настройки пайплайна: тест про провод, а не про конвертер в вакууме.
/// </summary>
public sealed class WireEnumTests
{
    ...
    /// <summary>Разбор строгий: ни имя CLR-члена, ни другой регистр, ни число старого провода не принимаются.</summary>
    ...
    /// <summary>Элементы массива накрывает глобальная фабрика: генератор конвертер им не ставит. Массив enum в теле — только фильтры счётчиков.</summary>
