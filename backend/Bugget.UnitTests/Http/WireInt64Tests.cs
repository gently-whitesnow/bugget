...
/// <summary>
/// Граница <c>long ↔ wire</c> для <c>Int64String</c>: <c>specs/contracts/shared.yaml</c> задаёт лексическую форму и
/// длину, здесь — точный диапазон runtime-границы. Значения по границам: <c>9007199254740993</c> — первое, что
/// клиент потерял бы в double; <c>9223372036854775807</c> и <c>...808</c> — верх Int64 и первое значение за ним.
/// </summary>
public class WireInt64Tests
{
    ...
    // Культура потока подменена намеренно: у неё свой разделитель групп, форматирование «по умолчанию» дало бы не канон.
    ...
    [InlineData("abc")]
    [InlineData("0x10")]
    // Не-ASCII цифры: long.TryParse их не берёт, но проверка формы стоит раньше и от парсера не зависит.
    [InlineData("١٢٣")]
