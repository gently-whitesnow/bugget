...
/// <summary>
/// Граница «результат бизнес-логики → HTTP-ответ» (ADR-0004). Проверяется напрямую, а не через контроллеры:
/// у синхронных перегрузок вызывающих может не быть, а фронт видит то же поведение.
/// Закреплено и превращение null: пустая коллекция уезжает массивом, пустой объект — объектом.
/// </summary>
