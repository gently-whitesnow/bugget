...
- В тёмной теме подпись тёмная (`{colors.dark-primary-content}` #0d1529): светлая давала
  3.26:1, а под курсором, когда фон светлеет, — 2.87:1. Тёмная — 4.95:1 и растёт на наведении.
...
- **`btn-primary` в светлой теме не проходит WCAG AA: 3.65:1** (белый на #009966, порог
  ...
  и `btn-primary` в тёмной теме уже починены сменой `*-content`, см. Components → Кнопки. Здесь так не выйдет:
  тёмная подпись на изумруде даёт 4.95:1 в покое, но под курсором фон темнеет, и на
  нажатии контраст падает до 3.75.
  - *Предложение:* затемнить светлый `--color-primary` с L 59.6% до 53% (#008453, белый
    даёт 4.73:1 в покое и больше на наведении). В тёмной теме оставить #009966: там
    `text-primary` на фоне с затемнённым изумрудом упал бы с 4.32 до 3.35.
  - *Риски:* бренд перестаёт быть одинаковым в двух темах, а это записано как принцип.
    Меняются и 21 `text-primary`, ссылки, `border-primary`, подложки `bg-primary/10`,
    кольцо фокуса — всё станет темнее. Нужен визуальный проход по обеим темам и
    согласование бренда; это решение не для правки кнопок.
