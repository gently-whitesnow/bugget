...
- `btn btn-info px-4 py-2`. Фон `{colors.info}`, подпись тёмная (`{colors.info-content}`
  #042e49) в обеих темах: белая на ярком голубом давала 2.04:1, тёмная — 6.31:1 в покое и
  4.53:1 на нажатии.
...
- **`--radius-selector` расходится между темами: 32px в светлой, 8px в тёмной.**
  Чекбоксы и тогглы в светлой теме круглые, в тёмной почти квадратные. `--size-field` и
  `--radius-field` уже выровнены (`0.21875rem` и `0.25rem`), селекторы редизайн кнопок не
  трогал.
  - *Предложение:* выбрать одно значение для обеих тем. 8px ближе к узкой шкале радиусов
    системы; 32px — пилюльная форма, которую система резервирует за круглыми элементами.
  - *Риски:* тогглы в светлой теме перестанут быть «пилюлями» — самое заметное изменение
    для пользователей. Этот же токен читают бейджи, `range`, `status` и `timeline`
    DaisyUI — бейджи у нас есть, их надо проверить отдельно.
