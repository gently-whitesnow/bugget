...
components:
  button-primary:
    ...
    height: 35px
  button-neutral:
    backgroundColor: color-mix(in oklab, {colors.base-100} 97%, {colors.base-content})
...
Бренд-цвет — изумруд `{colors.primary}`: #009966 в светлой теме, `{colors.dark-primary}`
#1dba84 в тёмной. **Оттенок и насыщенность одни и те же**, меняется только светлота. Он
держит идентичность, пока вокруг него переворачивается вся остальная палитра. В тёмной
теме бренд светлее, потому что средний изумруд там не читается ни с белой, ни с тёмной
подписью (см. Components → Кнопки).
...
- Двухтемная система со сквозным брендовым оттенком (`{colors.primary}` #009966 / #1dba84).
...
- **Primary** (`{colors.primary}` #009966 → `{colors.dark-primary}` #1dba84): бренд. Основные CTA,
...
- **Secondary** (`{colors.secondary}` #8fa0b8 → `{colors.dark-secondary}` #f43098): в светлой
  ...
  в кнопках secondary больше не используется — «Отменить» стала нейтральной.
...
- **Info** (`{colors.info}` #00bafe, идентичен в тёмной теме): информационные блоки,
  подсветка, статусы. В кнопках продукта не используется: «Сохранить» — `btn-primary`.
...
| `{rounded.selector}` | 32px | `--radius-selector` | Тогглы, бейджи DaisyUI — пилюли |
...
`{rounded.selector}` и размер селекторов (`--size-selector: 0.21875rem`) одинаковы в обеих
темах: тоггл и бейдж — пилюли, тоггл 21px высотой. Тот же токен скругляет DaisyUI
`.checkbox`, и на 32px он станет кругом, неотличимым от радио. Сейчас в продукте чекбоксов
DaisyUI нет; если появятся — дать им `{rounded.sm}` 4px явно.
...
| Компонент | Граница |
| --- | --- |
| `button-primary`, `button-error` | 1px в цвет собственного фона — шва не видно |
...
начертания без трекинга: Verdana разрежена специально для мелкого кегля, и поджимать её
нельзя (см. Принципы типографики). Вес 400 задан явно — иначе DaisyUI ставит 600 и подпись
становится жирной.
...
- В тёмной теме фон светлее (#1dba84), подпись тёмная (`{colors.dark-primary-content}`
  #0d1529): 7.25:1 в покое, на наведении и нажатии больше. Прежний #009966 со светлой
  подписью давал 3.26:1 и 2.87:1 под курсором, а с тёмной выглядел как заблокированная
  кнопка: средняя светлота не подходит ни белому, ни тёмному тексту.
  В светлой теме подпись пока белая и AA не проходит — см. Known Gaps.
- «Сохранить» (`SaveButton`) — тоже `btn-primary`: главное действие в системе одного цвета.
  Раньше это был неоновый `btn-info`, и рядом с зелёным «Создать» в интерфейсе было две
  главные кнопки разного цвета. Заблокирована, пока `isChanged === false`; при загрузке
  подпись заменяется на `loading-spinner`, ширина не скачет.

**`button-neutral`** — кнопка без цветового модификатора (`btn`, часто с `bg-base-100`).
Второстепенные действия: «Отменить» (`CancelButton`), «Показать ещё» в поиске, «Открыть
файл» и копирование cURL в превью вложений.
...
- Фон — `base-100` с примесью `base-content` 3%, под курсором 8%, на нажатии 13%. Не
  `base-200`: в тёмной теме он темнее страницы, и кнопка проваливалась. Примесь светлеет в
  тёмной теме и темнеет в светлой — кнопка всегда чуть приподнята над фоном и заметно
  светлее заблокированной (`base-content` 8%).
- «Отменить» раньше была `btn-outline btn-secondary`: в светлой теме серо-синяя подпись
  давала 2.67:1 и выглядела заблокированной, в тёмной маджентовая рамка кричала громче
  «Сохранить». Идёт в паре с «Сохранить» и блокируется по тому же `isChanged`.
...
**`button-error`** — деструктивные действия. `btn btn-error`. Подпись в светлой теме чисто
белая: розоватый `#fceeef` недотягивал до AA (4.06:1), белый даёт 4.59:1.
...
**Кнопка рядом с полем ввода — `btn-field`.** Поля зашиты в 42px, кнопки 35px и ниже, и
пара «поле + кнопка» в одном ряду выглядит ступенькой. `btn-field` поднимает кнопку до
высоты поля. Ряд — `flex flex-wrap gap-2`, поле `flex-1`, кнопка по ширине подписи;
сообщение об ошибке — **под рядом**, а не между полем и кнопкой.
...
- **`btn-primary` в светлой теме не проходит WCAG AA: 3.65:1** (белый на #009966, порог
  для обычного текста — 4.5:1; на наведении 4.35, на нажатии 4.84). `btn-info`, `btn-error`
  и `btn-primary` в тёмной теме уже починены, см. Components → Кнопки. Здесь так не
  выйдет: тёмная подпись на изумруде даёт 4.95:1 в покое, но под курсором фон темнеет, и на
  нажатии контраст падает до 3.75.
  - *Предложение:* затемнить светлый `--color-primary` с L 59.6% до 53% (#008453, белый
    даёт 4.73:1 в покое и больше на наведении). Тёмная тема уже на своём значении и не
    меняется.
  - *Почему важно:* «Сохранить» теперь тоже `btn-primary`, так что 3.65:1 у самой частой
    кнопки форм.
  - *Риски:* светлый бренд уйдёт от #009966, который узнаётся как цвет продукта. Темнее
    станут и 21 `text-primary`, ссылки, `border-primary`, подложки `bg-primary/10`, кольцо
    фокуса. Нужен визуальный проход по светлой теме и согласование бренда; это решение не для правки кнопок.
- **В тёмной теме бренд и success почти одного цвета**: `{colors.dark-primary}` #1dba84
  и `{colors.dark-success}` #00d390 — один оттенок (163°), разница только в светлоте 70% и
  76%. Так было и раньше, но после осветления бренда они сблизились.
  - *Предложение:* увести success по оттенку в сторону жёлто-зелёного (~150°, как в светлой
    теме), бренд не трогать.
  - *Риски:* зелёные статусы в тёмной теме сменят характер; проверить бейджи и аналитику.
- **`text-info` на белом — 2.22:1** (21 употребление). Смена `info-content` чинит только
  подпись на сплошной заливке, а не голубой текст на фоне страницы.
  - *Предложение:* отдельный токен для текста (или `text-info` → затемнённый оттенок в
    светлой теме), фон `bg-info/10` оставить как есть.
  - *Риски:* плашки и статусы аналитики на `bg-info` и `text-info` визуально разъедутся;
    нужен обход всех 21 места.
- **Подпись кнопки — 13px вместо прежних 14px**, запас по читаемости меньше. После
  починки контраста это не блокер, но `btn-primary` в светлой теме на 13px — самый слабый
  случай.
  - *Предложение:* если бренд не затемняем, вернуть кнопкам 14px хотя бы для `btn-primary`.
  - *Риски:* кнопки снова станут шире, в плотных тулбарах возможны переносы; и в шкале
    появится развилка размера по цвету кнопки.
- **Радиус поля 4px строже, чем 6px у Linear, на который ориентировался редизайн.** Это
  осознанная цена чистого угла на DPR 1 (см. Shapes → «Почему у поля 4px»).
  - *Предложение:* ничего не делать, пока в аудитории заметна доля мониторов с DPR 1.
    Вернуться, если перейдём на рамку через `outline` / SVG или DPR 1 перестанет быть
    массовым.
  - *Риски:* 6px без смены техники рамки возвращает «просвечивающий» угол на обычных
    мониторах.
- **`btn-xs` — 21px, `btn-sm` — 28px** в обеих темах: для одиночных тач-целей не годятся
  (минимум 44px), как и до редизайна. Раньше в тёмной теме они были 24 и 32px, что тоже
  ниже минимума.
  - *Предложение:* на `(pointer: coarse)` расширять кликабельную область псевдоэлементом
    (`::after` с отрицательным `inset`) до 44px, не меняя видимый размер.
  - *Риски:* соседние маленькие кнопки в строке начнут перекрывать зоны друг друга —
    нужен минимальный `gap`. Класс `.tooltip` на самой кнопке сломает приём: тултип рисует
    себя через `::before` / `::after` (сейчас таких сочетаний нет).
...
- **`{colors.secondary}` меняет роль между темами**: нейтральный серо-синий #8fa0b8 в
  светлой, насыщенная маджента #f43098 в тёмной. Из кнопок продукта он ушёл вместе с
  «Отменить», но роль токена так и не определена.
...
- Токены `--depth: 1` и `--noise: 0` в теме оставлены дефолтными DaisyUI. У кнопок
  `--depth` переопределён в 0 (см. Components → Кнопки); поля ввода, чекбоксы и алерты
  свою объёмность от DaisyUI по-прежнему получают.
