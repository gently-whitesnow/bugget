...
Визуальный контракт — [DESIGN.md](DESIGN.md): палитра, типографика, отступы, радиусы и
правила компонентов. Источник правды по значениям — `src/shared/styles/tailwind.css` и
`src/app/global.css`; DESIGN.md собран из них и обязан обновляться вместе с ними.

Файл написан в открытом формате DESIGN.md, поэтому проверяется машиной, а не глазами:
`npx @google/design.md lint DESIGN.md` (ошибок быть не должно; про ожидаемые
предупреждения — в разделе Known Gaps самого файла). Откуда взялся формат и что в нём
намеренно нарушено — в шапке DESIGN.md.
