...
// HTTP перешёл на строки (ADR-0013), SignalR остался числовым (ADR-0007); типами
// ошибка на шве не видна, поэтому проверяется весь диапазон каждого enum'а.
...
// Неизвестное число — расхождение контрактов: «нулевое» значение замаскировало
// бы его чужим статусом, поэтому шов падает громко.
...
// `tg_beta_tester` был на проводе, но не в константах фронта, и внешний автор
// разрешался как внутренний пользователь.
