 ...
 *
 * Массив кладётся повторяющимся ключом
 * (`reportStatuses=backlog&reportStatuses=fix`) — так его читает бекенд и так его
 * писал рукописный код. Через `params` axios сериализовал бы его как
 * `reportStatuses[]=backlog`, то есть сменил бы провод.
 *
