...
// Проверяется сама краснота правила `frontend-lint`: правило, которое ничего
// не ловит, выглядело бы как выполненный инвариант.
...
describe("гейт прямых вызовов путей users", () => {
  ...
  it("краснеет на пути без префикса модуля — его дописывает интерсептор", () => {
    // `/v1/...` доезжает до бекенда тем же адресом, что `/api/users/v1/...`.
