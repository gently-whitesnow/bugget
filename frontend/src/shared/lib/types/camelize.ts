 ...
 *
 * Правило границы одно на весь фронт (ADR-0009):
 *
 ...
 *
 * Гейт `frontend-contracts` дополнительно требует, чтобы имена полей тела были
 * каноничным `snake_case`, переживали round-trip `wire → camelCase → wire` и
 * чтобы в теле не было свободных словарей
 * (`scripts/quality/frontend-case-roundtrip.py`): что не выражается этой
 * проекцией, то в контракт не принимается.
 */
