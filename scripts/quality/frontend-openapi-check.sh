...
#   * имя поля тела не переживает round-trip wire -> camelCase -> wire, то есть
#     не выражается проекцией фронта (frontend-case-roundtrip.py).
#
...
# Round-trip имён полей тела: wire -> camelCase -> wire. Имя, которое проекция
# фронта не возвращает обратно, ломает провод молча, поэтому контрактная
# проверка останавливается на нём до того, как контракт начнут использовать.
