...
знака, ведущих нулей и разделителей. OpenAPI выражает лексическую форму компактным
`pattern` и ограничивает длину 19 символами; точный диапазон
`0..9223372036854775807` проверяется на HTTP-границе. Поле обязательное и не
допускает `null`.
...
- Штатные `backend-contracts` и `frontend-contracts` удерживают воспроизводимость
  генерации из общей схемы. Точный диапазон намеренно не кодируется в generated
  regex и остаётся ответственностью единой HTTP-границы.
